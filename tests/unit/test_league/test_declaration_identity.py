...
def test_identity_urls_refuse_embedded_credentials_or_query_data() -> None:
    with pytest.raises(ValueError, match="GitHub"):
        replace(_identity(), cop_repo="https://token@github.com/example/cop")
    with pytest.raises(ValueError, match="/mcp"):
        replace(_identity(), cop_mcp_url="https://cop.example/mcp?token=hidden")
    with pytest.raises(ValueError, match="GitHub"):
        _identity().__class__(
            group_id=GROUP_A,
            group_name="Alpha Team",
