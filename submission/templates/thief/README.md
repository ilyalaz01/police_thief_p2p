# Police-Thief P2P — Thief

This is the Thief role repository for the university Police-Thief P2P project. It contains the
same verified shared engine, protocol runtime, SDK, audit/replay, GUI, tests, and release tooling
as the Police repository; the role identity and runtime policy below are the deliberate differences.
...
`RandomLegalThief` samples uniformly from the role-legal action list with a deterministic seed.
The simulator validates the selected action again before applying it. The policy is deliberately
simple and receives no Police coordinate; it is the observed integration default, not a claim of
optimality or a newly promoted champion.
...
it does not train a model or promote this default. See the
...
uv run python -m police_thief_lab.viewer_cli live \
  --snapshot artifacts/thief-live.json --host 127.0.0.1 --port 8765
```
