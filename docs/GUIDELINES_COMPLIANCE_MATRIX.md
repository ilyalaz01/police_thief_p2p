...
| 6 Configuration/secrets | COMPLIANT | Strict loader, tracked JSON example, `.env-example`, ignore rules, sanitized scanner | Current offline/self-test modes require no credentials; future secret use remains environment-only | CFG-001 | Phase 4D2A compatibility and secret-scan tests |
| 7 Version tracking | COMPLIANT | pyproject/package/config `1.0.0`; strict config schema `1.0`; profile schemas remain separate | Unsupported schema/package versions fail before peer side effects | CFG-001 | Phase 4D2A coherence and rejection tests |
