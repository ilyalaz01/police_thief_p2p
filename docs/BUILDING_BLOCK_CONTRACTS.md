...
`PoliceThiefSDK` is a stateless composition root. Setup constructs seven concern-specific services;
...
| `LeagueSDK` | Complete operator-supplied Appendix-B values, two group identities, six sealed slots, commits and a new output path | Pending/locked config bytes, canonical series IDs, six two-process loopback results, two artifact sets and a sanitized aggregate | `run_localhost_series` is explicitly `UNCOUNTED_LOCALHOST_SELF_TEST`; all other league operations remain stateless/offline | Exact schema/fixed/minimum fields, matching profile/hash approval, role identity, audit/replay, official score, artifact join and six-game completeness fail closed before atomic publication | Failed peers publish no final bundle; a tied series remains blocked; no SDK call grants public transport, Gmail, opponent, reporting or counted authorization |
