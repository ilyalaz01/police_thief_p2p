...
| `LeagueSDK` | Complete operator-supplied Appendix-B values, two group identities, six sealed slots and already-scored rows | Pending/locked config bytes, canonical series IDs, schedules, validated provenance and raw aggregate | Stateless and offline; the injected coordinator starts no peer itself | Exact schema/fixed/minimum fields, matching profile/hash approval, role identity, official score and six-game completeness fail closed | A tied series remains blocked; no SDK call grants network, Gmail, opponent, reporting or counted authorization |
