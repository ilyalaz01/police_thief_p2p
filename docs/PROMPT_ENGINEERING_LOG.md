# Prompt Engineering Log

> Retrospective baseline created after the validated prototype.
> These documents did not exist before the prototype and do not claim otherwise.

## Retrospective limitation

No complete historical prompt log existed. Verbatim historical prompts, approvals, dates, and
iterations cannot be reconstructed and are not claimed. This log contains only high-level process
summaries supported by existing phase reports. It excludes private chat bodies, personal
messages, credentials, partner instructions, live endpoints, and operational nonces.

The known process mistake is that implementation preceded formal PRD/PLAN/TODO approval. The
corrective action is this honest retrospective baseline plus a rule that every future significant
prompt links an approved requirement, plan/ADR as applicable, TODO ID, verification, and resulting
commit/PR.

## Evidence-derived summaries

| Phase evidence | Goal summary | Lesson supported by evidence |
|---|---|---|
| Phase 1/core reports and tests | Establish deterministic rules, legal observations and replay. | Higher-authority edge rules and hidden-state boundaries need explicit regression tests. |
| Phase 2/3 reports and experiments | Compare baselines, belief and search under seeded evaluation. | Separate research proposals from frozen competitive choices; retain seeded evidence. |
| Interop verification/Phase 4A | Resolve byte contracts and complete local two-peer flow. | Similar-looking serialization domains and reference/book behavior must remain named. |
| Phase 4B/4C reports | Validate transport gates and prepare blocked human worksheets. | Technical validation never substitutes for bilateral approval or authorization. |
| Phase 4D0 audit | Recover requirements, architecture, quality and task governance. | Retrospective documentation must state its timing and leave technical gaps open. |
| Phase 4D1A test split | Mechanically split oversized interoperability tests under REF-001. | Path-normalized collection and AST manifests make organization-only moves independently verifiable. |
| Phase 4D1A.1 correction | Repair live test references and specify the AST digest preimage. | Governance should validate live backticked test paths, and evidence hashes need exact framing rules. |
| Phase 4D1B belief split | Characterize the belief contract, then extract three private pure helpers. | Pin outputs, signatures, exceptions, exports, and diagnostics before moving strategy calculations. |
| Phase 4D1C artifact split | Characterize schema, writer, hash, and consensus bytes before moving encoding helpers. | Introspected signatures and AST/constant manifests catch context-sensitive movement errors such as annotation evaluation. |
| Phase 4D1C.1 correction | Restore normal formatting while preserving production and test AST manifests. | Line limits need explicit anti-compression checks, including physical length and unsuppressed lint gates. |
| Phase 4D1D Phase 3B split | Pin 27 seeded model/usage/depth combinations, public API, diagnostics and errors before extracting models, replies and scoring. | Strategy refactors need output-level vectors, not only ordinary unit coverage. |
| Phase 4D1D runtime split | Pin runtime signatures/conversions and rerun isolated-process, terminal, audit/replay and schema 1.1 flows while separating responsibilities. | Unicode defaults and module-global patch points are observable contracts; compare them explicitly before accepting a structural split. |
| Phase 4D2A operational config | Commit a failing config-contract test, then add a strict loader, CLI preflight, safe example and value-redacting scan under CFG-001/ADR-006. | Keep operational metadata outside MatchProfile bytes; strict version/mode rejection before side effects is safer than implicit migration. |
| Phase 4D2B SDK facade | Commit a failing single-entry inventory, then compose six concern-specific services and reduce CLI to typed delegation under SDK-001/ADR-003. | Alias proven implementations instead of wrapping behavior; separate facade completeness from future Gatekeeper policy. |
| Phase 4D2C API Gatekeeper | Commit a failing centralized-call contract, then add file-loaded rates, bounded FIFO/backpressure/drain/value-free metrics and gate every FastMCP attempt under API-001/ADR-004. | Preserve higher-authority retry ownership while gating each attempt; bound inbound mailboxes without dropping or reordering messages. |
| Phase 4D3 test quality | Reorganize tests into unit/integration/system layers and add critical-path/coverage evidence without changing collection intent. | Compare normalized collection and behavior before treating a test move as semantics-preserving. |
| Phase 4D4 concurrency | Reproduce Gatekeeper close/admission races, then make capacity, shutdown and worker reaping deterministic. | Concurrency acceptance needs explicit ownership, exact capacity, race barriers and eventual cleanup evidence. |
| Phase 4D5 partner release integration | Port the partner-owned offline-ops boundary onto current main, preserve authorship, repair stale paths/encoding, and validate the same CI gate. | Clean-port stale work rather than rebasing through unrelated architecture; test contracts must normalize cross-Python AST representation. |
| Phase 4D6 CLI/manual | Re-read official and guideline sources, add RED contracts for discoverable safe CLI help and a complete manual, then expose higher-authority submission blockers. | Guideline closure must not hide official two-repository, GUI/Replay, counted-game, or reporting requirements; validate publishable state in a clean checkout while preserving ignored local evidence. |
| Phase 4D7B Replay shell | Derive a role-safe view boundary and standalone post-game Replay app from official Chapter 7, then verify it on synthetic tamper cases and a retained completed-game log. | Keep live partial truth structurally separate from post-game truth; verify before rendering and never embed revealed nonces in the HTML app. |
| Phase 4D7C Live GUI | Add RED role-safe feed/runtime/loopback contracts, connect observations to an atomic bounded feed, visually review localhost Live/Replay apps, and retain synthetic screenshots. | Treat presentation as another privacy boundary: use an exact allow-list schema, reject extra fields, bind only to loopback, and preserve protocol/profile/strategy bytes. |
| Phase 4D7A cost/ISO integration | Review independently implemented measurements and assessments, reproduce their tests, then retain authorship while correcting units, billing claims, path privacy and the post-GUI ISO state through transparent RED/GREEN commits. | Independent contribution still requires evidence review; preserve contributor commits and correct findings without rewriting history. |
| Phase 4D8 package contracts | Inventory every editable production symbol, commit RED failures, add inline/external contracts, audit exports/imports/metadata, and formalize extension applicability without changing frozen files. | Exact higher-authority byte freezes need external documentation rather than cosmetic hash drift; typed extension points are safer than a dynamic competitive-runtime plugin loader. |
| Phase 4D9 research publication | Preregister a bounded paired OAT design, generate public-safe data/figures through the public SDK, prove byte-identical regeneration, then add a RED correction requiring embedded figures, captions and explicit hypothesis assessment. | Reproducibility is not only stable data: an analysis artifact must connect preregistered claims, uncertainty, accessible figures and limitations without converting local screening into a competitive-policy decision. |
| Phase 4D10 local cost/capacity | Preregister fixed warm-up, timing and memory seed ranges; add pure arithmetic and source-tree guards; retain one-machine local simulator samples; and reconcile living cost/ISO/readiness documents. | Measure applicable runtime facts before forecasting; separate unretained warm-ups, timing without tracer overhead, and Python-allocation probes, while leaving prices, RSS, public latency and parallel scaling explicitly unclaimed. |
| Phase 4D11 Git/release governance | Inspect accepted first-parent history, contributor attribution and annotated tags; commit a RED full-history/audit contract; enable tag-aware CI; and reconcile TODO/compliance/readiness without creating a final tag. | Preserve honest history and record review exceptions instead of fabricating them; a final submission tag is a content-approval action, not evidence that can be manufactured during a governance audit. |
| Phase 4D12A role content boundary | Derive Rule 49/50 repository needs from official evidence, commit a RED tracked-file policy contract, select a safe common content set, and separate exporter bytes from history/submodule/final-URL operations. | A deterministic snapshot is not a Git-history migration; preserve contributor provenance, exclude synthetic scanner fixtures explicitly, and describe the current Thief default without silently promoting a new policy. |
| Phase 4D12B role README assembly | Commit RED contracts for future-root role manuals and ordered assembly, then add standalone Police/Thief overlays with blocking URLs and a history-preserving runbook. | Role READMEs are release inputs, not decoration: test policy identity, required layout, safe commands, counterpart blocking, provenance, submodule restoration, and the final human gate explicitly. |
| Phase 4D12C compliance freshness | Audit the live TODO/matrix after D12B, commit a RED stale-claim detector, then replace obsolete counts/cutoffs without promoting historically partial ratings. | A compliance ledger must age safely: derive volatile evidence, link the latest accepted audit, distinguish irreparable history from current implementation, and never erase a limitation to improve presentation. |
| Phase 4D12D official requirements audit | Re-read the official PDF, complete guidelines and authority baseline; inspect architecture and living evidence; add RED cross-document contracts; then reconcile only demonstrated stale claims and expose separate LGE/MAIL/SUB/HUM delivery gates. | Generic-guideline completion is not league-delivery completion: keep official config domains named, distinguish missing implementation from authorization, and never convert a green audit into counted-readiness evidence. |
| Phase 4D13B localhost series | Extend the accepted six-slot entry contract with six unchanged two-process loopback games, twelve audit/replay checks, atomic dual artifact assembly, and official checker/join validation while stopping before external operations. | A series adapter must reuse the proven one-game boundary, keep full Appendix-B bytes separate from flat schema-1.1 terms, publish nothing after a failed peer, and label synthetic matching inputs as self-test rather than bilateral approval. |
| Phase 4D14A role candidate snapshots | Add a RED two-role assembly contract, compose the partner-authored exporter through a separate policy layer, reproduce both exact role trees atomically, and scan their bytes while retaining every external hard stop. | Snapshot bytes and Git history are different evidence: prove deterministic selection and README-only overlay first, then create history-preserving role branches and restore the pinned submodule before claiming independent repository gates. |
| Phase 4D14B role repository gate | Build real temporary Git role candidates in RED/GREEN tests, exclude construction-only machinery from final trees, and verify ancestry, exact bytes, academic README, gitlink pin, privacy and tag absence through a read-only CLI. | A passing shared snapshot is insufficient for Rule 49/50 delivery; the final gate must prove repository history and dependency identity without creating a remote, link, tag, or external authorization. |
| Phase 4D14C local role candidates | Assemble one Police and one Thief history-preserving branch from the exact accepted source, run repository and full quality gates independently, correct the post-evidence file count, and stop before URL/remotes/tags. | Release evidence must distinguish a local Git candidate from a published repository; expected skips need named redistribution/evidence reasons, and a parallel localhost warning must be replaced by a clean sequential result. |
| Phase 4D14D approved role URLs | Commit RED reciprocal-link/publication contracts, bind the operator-approved role URLs in policy and manuals, record the two empty public repositories, and preserve separate tag/external-operation hard stops. | Creating empty repositories is not publication acceptance: integrate only an accepted shared source, rerun both role gates, push non-forcibly, and verify each public link and CI before claiming Rule 49 delivery. |
| Phase 4D14E role publication | Commit a RED exact-publication evidence contract, merge the accepted shared URL source into both history-preserving role branches, resolve only policy-excluded construction paths, rerun independent gates, publish non-forcibly, and verify public links/CI. | Clean-clone CI can depend on published historical tag objects as well as branch bytes: reproduce failure from the public remote, publish only the unchanged accepted baseline tag, rerun both jobs, and keep final submission tags separately blocked. |

## Template for every future significant prompt

Copy one entry; summaries are preferred over sensitive prompt bodies.

### PE-YYYY-NNN — Short title

- Date: YYYY-MM-DD (actual entry date)
- Author/operator: role or approved public identifier
- Context: requirement/TODO/ADR/evidence links
- Goal: measurable intended outcome
- Constraints: authority, frozen boundary, privacy, scope and hard stops
- Output: files/results produced; no sensitive body
- Verification: exact `uv` commands and observed counts/status
- Refinements: concise iteration summaries and why they were needed
- Decision: accepted/rejected/proposed plus rationale and approver evidence
- Commit/PR link: immutable identifier when one exists
- Lessons learned: reusable prompting/process guidance
